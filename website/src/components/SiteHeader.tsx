...
export const SocialIcons = ({ isWhite = false }: { isWhite?: boolean }) => {
    const socialLinks = [
        ...
        {
            href: "https://open.spotify.com/user/y09l1m30h1s75djr7oaiw3oob?si=89c7e0945dc34bb5",
            icon: (
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
        ...
        <div className="fixed inset-0 z-50 bg-[rgb(199,78,38)] overflow-y-auto overflow-x-hidden font-bradford">
                            ...
                            content={
                                <>
                                    {NAV_DATA.corporate.map(item => (
                                        <Link key={item.to} to={item.to} className={linkClass} onClick={onClose}>{item.label}</Link>
                                    ))}
                                </>
                            }
                        />
            ...
            <div className="block lg:hidden px-6 pb-10 max-w-[600px] mx-auto flex flex-col h-[calc(100vh-64px)]">
                ...
                <div className="mb-1 flex flex-col border-t border-white/20">
                    <Link to="/hakkimizda" onClick={onClose} className="py-2.5 border-b border-white/20 text-white font-bradford text-lg hover:pl-2 transition-all">Hakkımızda</Link>
                    <Link to="/hakkimizda" onClick={onClose} className="py-2.5 border-b border-white/20 text-white font-bradford text-lg hover:pl-2 transition-all">İletişim</Link>
                </div>
                ...
                <div className="flex flex-col">
                    ...
                    >
                        <Link to="/category?category=Güncel" className={linkClass} onClick={onClose}>Güncel</Link>
                        <Link to="/category?category=Tarih" className={linkClass} onClick={onClose}>Tarih</Link>
                        <Link to="/category?category=Kuram" className={linkClass} onClick={onClose}>Kuram</Link>
                        <Link to="/category?category=Felsefe" className={linkClass} onClick={onClose}>Felsefe</Link>
                        <Link to="/category?category=Kültür-Sanat" className={linkClass} onClick={onClose}>Kültür-Sanat</Link>
                        <Link to="/category?category=Dünya" className={linkClass} onClick={onClose}>Dünya</Link>
                        <Link to="/category?category=Spor" className={linkClass} onClick={onClose}>Spor</Link>
                    </MobileAccordion>
                    ...
                    >
                        <Link to="/tematik/bir-olay-bir-kavram" className={linkClass} onClick={onClose}>Bir Olay Bir Kavram</Link>
                        <Link to="/tematik/portre" className={linkClass} onClick={onClose}>Portre</Link>
                        <Link to="/tematik/nato" className={linkClass} onClick={onClose}>NATO</Link>
                    </MobileAccordion>
                    ...
                    >
                        <Link to="/dosyalar/cumhuriyet" className={linkClass} onClick={onClose}>Cumhuriyet</Link>
                    ...
                    >
                        <Link to="/sayi/505" className={linkClass} onClick={onClose}>Son Sayı: 505</Link>
                        <Link to="/sayi/504" className={linkClass} onClick={onClose}>Sayı 504</Link>
                        <Link to="/sayi/502" className={linkClass} onClick={onClose}>Sayı 502</Link>
                        <Link to="/sayilar" className="text-white/60 text-sm font-bold mt-2 block" onClick={onClose}>Tüm Arşiv →</Link>
                    </MobileAccordion>
...
export const SiteHeader = () => {
    ...
    // Scroll Progress Hook (only meaningful on article pages)
    ...
    useEffect(() => {
        // Reset isTop on route change so non-article pages always show header
        setIsTop(true);
        setIsVisible(true);
    }, [location.pathname]);
    ...
    // Transparent only on article/category/tematik/dosyalar pages at top (white text over dark hero)
    const isEffectiveTransparent = isTransparentPage && isTop && !isHovered;

    return (
        <>
            {/* 1. READING PROGRESS BAR — only on article pages */}
            {isArticlePage && (
                <motion.div
            ...
            <header
                ...
                className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ease-in-out
                    ...
                    ${isEffectiveTransparent ? 'bg-transparent hover:bg-white/10' : 'bg-white'}
                `}
            >
                <div className="px-4 sm:px-8 md:px-16 lg:px-24 xl:px-[140px]">
                    <div className="relative flex items-center justify-between h-20 max-w-[1200px] mx-auto">
